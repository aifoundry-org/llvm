...
example(9, x, y, x, x, y, y, x, 1<<19)
    ...
    // CHECK-NEXT:	mov.m.x	m0, zero, 172
    // CHECK-NEXT:	faddi.pi	ft0, ft0, 1
    // CHECK-NEXT:	mov.m.x	m0, zero, 202
    // CHECK-NEXT:	faddi.pi	ft0, ft0, 2
    // CHECK-NEXT:	fsq2	ft0, 0(a0)
    // CHECK-NEXT:	ret
    // CHECK:build5:
    ...
    // CHECK-NEXT:	mov.m.x	m0, zero, 32
    // CHECK-NEXT:	faddi.pi	ft0, ft0, 524284
    // CHECK-NEXT:	mov.m.x	m0, zero, 42
    // CHECK-NEXT:	faddi.pi	ft0, ft0, 2
    // CHECK-NEXT:	mov.m.x	m0, zero, 44
    // CHECK-NEXT:	faddi.pi	ft0, ft0, 1
    // CHECK-NEXT:	mov.m.x	m0, zero, 47
    // CHECK-NEXT:	faddi.pi	ft0, ft0, 524288
