...
static DecodeStatus DecodeFPR256RegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  Register Reg = RISCV::F0_PS + RegNo;
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
DecodeStatus RISCVDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
  ...
  if ((Bytes[0] & 0x3) == 0x3) {
    ...
    if (STI.getFeatureBits()[RISCV::FeatureEsperanto]) {
      LLVM_DEBUG(dbgs() << "Trying Esperanto table (32-bit Instruction):\n");
      // Calling the auto-generated decoder function.
      Result = decodeInstruction(DecoderTableEsperanto32, MI, Insn, Address,
                                 this, STI);
